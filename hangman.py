...
class HangmanGame:
  def __init__(self, master):
      ...
      self.master.configure(bg='lavender blush')
      self.word_list = ["PYTHON", "COMPUTER", "DATA", "CODING", "JAVA", "ALGORITHM", "PROGRAM", "GAMES"]
  ...
  def initialize_gui(self):
      ...
      self.word_display = tk.Label(self.master, text="_ " * len(self.secret_word), font=("Helvetica", 30), bg='lavender blush')
      self.word_display.pack(pady=(40, 20))
      self.reset_button = tk.Button(self.master, text="Reset Game", command=self.reset_game, width=20, height=2, bg=button_bg,
  ...
  def check_game_over(self):
      if set(self.secret_word).issubset(self.correct_guesses):
          self.display_game_over_message("Congratulations, you win!")
  ...
  def display_game_over_message(self, message):
      ...
      self.game_over_label = tk.Label(self.master, text=message, font=stylish_font, fg="#b997e8", bg='lavender blush')
